...
public class Disclaimer : MonoBehaviour
{
    ...
    private void Awake()
    {
        // TEMP, to test disclaimer functionality. remove when done
        //PlayerPrefs.SetInt("AudioCheckSeen", 0);

        // singleton code
        if (Instance == null) { Instance = this; }
        else if (Instance != this) { Destroy(this); }

        if (PlayerPrefs.GetInt("DisclaimerSeen") == 1 && PlayerPrefs.GetInt("AudioCheckSeen") == 1)
        {
            DisableDisclaimer();
        }
    }
    ...
    public void SeenAudioCheck()
    {
        PlayerPrefs.SetInt("AudioCheckSeen", 1);
    }
