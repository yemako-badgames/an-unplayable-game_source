...
public class Settings : MonoBehaviour
{
    ...
    public bool tvStaticMuted { get; private set; } = false;

    public bool useAccessibleFont { get; private set; } = false;
    public UnityEvent fontChanged;
    ...
    private void Awake()
    {
        ...
        // initialize tv static mute setting if it has been changed in the past
        if (PlayerPrefs.HasKey("TvStaticMuted"))
        {
            // convert int to bool
            if (PlayerPrefs.GetInt("TvStaticMuted") == 1)
            {
                tvStaticMuted = true;
            }
            else { tvStaticMuted = false; }
        }


        // initialize accessible font setting if it has been changed in the past
        if (PlayerPrefs.HasKey("UseAccessibleFont"))
        {
            // convert int to bool
            if (PlayerPrefs.GetInt("UseAccessibleFont") == 1)
            {
                useAccessibleFont = true;
            }
            else { useAccessibleFont = false; }
        }
    ...
    public void SetTvStaticMute(bool isMuted)
    {
        tvStaticMuted = isMuted;

        if (tvStaticMuted)
        {
            PlayerPrefs.SetInt("TvStaticMuted", 1);
        }
        else { PlayerPrefs.SetInt("TvStaticMuted", 0); }
    }

    public void SetAccessibleFont(bool useFont)
    {
        useAccessibleFont = useFont;


        if (useAccessibleFont)
        {
            PlayerPrefs.SetInt("UseAccessibleFont", 1);
        }
        else { PlayerPrefs.SetInt("UseAccessibleFont", 0); }
